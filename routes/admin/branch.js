...
branchRouter.post("/create-branch", async function (req, res) {
  let colte = req.body;
  delete colte._id;
  await BranchModel.countDocuments({
    code: req.body.code,
...
branchRouter.post("/update-branch", async function (req, res) {
  await BranchModel.countDocuments({ _id: req.body._id }).then(
    async (count) => {
      if (count === 0) {
        res.send({ status: false, message: "Branch not found" });
      } else {
        BranchModel.findOneAndUpdate({ _id: req.body._id }, req.body).then(
          (Branch) => {
