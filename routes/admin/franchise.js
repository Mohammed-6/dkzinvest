...
franchiseRouter.post("/create-franchise", async function (req, res) {
  let colte = req.body;
  delete colte._id;
  await FranchiseModel.countDocuments({
    code: req.body.code,
...
franchiseRouter.post("/update-franchise", async function (req, res) {
  await FranchiseModel.countDocuments({ _id: req.body._id }).then(
    async (count) => {
      if (count === 0) {
        res.send({ status: false, message: "Franchise not found" });
      } else {
        FranchiseModel.findOneAndUpdate({ _id: req.body._id }, req.body).then(
          (franchise) => {
            res.send({
              status: true,
...
franchiseRouter.post("/delete-franchise", async function (req, res) {
  await FranchiseModel.countDocuments({ _id: req.body._id }).then((count) => {
    if (count === 0) {
      ...
      FranchiseModel.findOneAndDelete({ _id: req.body._id }).then(
        (franchise) => {
