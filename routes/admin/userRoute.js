...
async function getUserCount() {
    ...
    return await UserModel.countDocuments({}).then(async (count) => {
        if(count > 0) {
        return await UserModel.findOne({}).sort({newId:-1}).then(function (user) {
            // console.log(user);
            return user.newId;
        });
        }else{
...
userRouter.post('/create-user', async function (req, res) {
    ...
    await bcrypt.hash(colte.password, 10).then(async function (hash) {
        ...
        colte.password = hash;
        await UserModel.countDocuments({email: req.body.email, phone: req.body.phone}).then((count) => {
            if(count!==0){
                res.send({status: false, message: "User already exists"});
            }else{
                UserModel.create(colte).then((user) => {
                    res.send({status: true, message: "User created successfully"});
                }).catch((error) => {
                    res.send({status: false, message: "Error", data: error, dd: colte});
                });
            }
        })
    })
    ...
    await UserModel.countDocuments({_id: req.body._id}).then((count) => {
        if(count===0){
            ...
            UserModel.findOne({_id: req.body._id}).then((user) => {
                res.send({status: true, message: "User find", data: user});
            })
        }
    })
});
...
userRouter.post('/update-user', async function (req, res) {
    ...
    await UserModel.countDocuments({_id: req.body._id}).then(async (count) => {
        if(count===0){
            ...
            UserModel.findOneAndUpdate({_id: req.body._id}, alldata).then((user) => {
                ...
                await bcrypt
                    ...
                    if (( result)) {
                        if(req.body.password === req.body.conpassword){
                            ...
                            existsUser._id, {password: alldata.password}).then(function (mdata) {
                                res.send({
                                status: true,
